...
import { drawerConfigPath, drawersDir, tmpConfigDir, tmpDrawersConfigPath } from './paths'
...
function getDrawerName(path: string = '') {
  ...
  const drawerName = fileName.replace(/\.tsx$/, '').replace(/\s/g, '_')
  return drawerName
}
...
function formatDrawers(drawers: string[]) {
  return drawers.map(item => {
    ...
    const drawerImportPath = item.replace(/^src\/drawers/, '@drawers').replace(/\.tsx$/, '')
