...
import { modalConfigPath, modalsDir, tmpConfigDir, tmpModalsConfigPath } from './paths'
...
function getModalName(path: string = '') {
  ...
  const modalName = fileName.replace(/\.tsx$/, '').replace(/\s/g, '_')
  return modalName
}
...
function formatModals(modals: string[]) {
  return modals.map(item => {
    ...
    const modalImportPath = item.replace(/^src\/modals/, '@modals').replace(/\.tsx$/, '')
