...
import { routerConfigPath, pagesDir, tmpConfigDir, tmpRouterConfigPath } from './paths'
...
function getPageName(path: string = '') {
  ...
  const pageName = fileName.replace(/\.tsx$/, '').replace(/[\s\.\-\^\$]/g, '_')
...
function formatPages(pages: string[]) {
  return pages.map(item => {
    ...
    const pageImportPath = item.replace(/^src\/pages/, '@pages').replace(/\.tsx$/, '')
    if (item === HOME_PAGE) {
      routerPath = '/'
    } else {
